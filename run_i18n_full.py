...
with open(os.path.join(BASE, 'data/mockData.js'), 'w', encoding='utf-8') as f:
    f.write(MOCKDATA)
print('mockData.js written')

# ── 2. Update pages to use getLocalizedPatientData ─────────────────────────
page_patches = [
    # SurgeryInfo
    ('pages/SurgeryInfo.jsx',
     "import { patientData }",
     "import { patientData, getLocalizedPatientData }"),
    ('pages/SurgeryInfo.jsx',
     "const { surgery, recoveryGuide } = patientData",
     "const { lang } = useLang()\n  const { surgery, recoveryGuide } = getLocalizedPatientData(lang)"),

    # MedicineInfo
    ('pages/MedicineInfo.jsx',
     "import { patientData }",
     "import { patientData, getLocalizedPatientData }"),

    # FollowUpSchedule
    ('pages/FollowUpSchedule.jsx',
     "import { patientData }",
     "import { patientData, getLocalizedPatientData }"),
    ...
    ('pages/HospitalDashboard.jsx',
     "import { hospitalPatients",
     "import { hospitalPatients, getLocalizedHospitalPatients"),

    # GlobalIntelligence
    ('pages/GlobalIntelligence.jsx',
     "import { globalPatterns",
     "import { globalPatterns, getLocalizedGlobalPatterns"),
]

for rel, old, new in page_patches:
    path = os.path.join(BASE, rel)
    try:
        with open(path) as f: c = f.read()
        if old in c:
            with open(path,'w') as f: f.write(c.replace(old, new, 1))
            print(f'Patched import: {rel}')
        else:
            print(f'NOT FOUND: {old[:50]} in {rel}')
    except FileNotFoundError:
        print(f'File not found: {rel}')

print('\nDone. Now update each page to call getLocalizedPatientData(lang) where patientData is destructured.')
